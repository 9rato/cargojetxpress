...
if (isset($_GET['submit'])) {
    // Check if the 'tid' parameter is set
    if (isset($_GET['tid'])) {
        $tid = $_GET['tid'];

        // Perform your logic to check payment information using the $tid variable
        $sql = "SELECT * FROM inpayment WHERE trackid = '$tid'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            echo "Tracking ID: " . $row['trackid'] . "<br>";
            echo "Amount: " . $row['amount'] . "<br>";
            echo "Date: " . $row['date'] . "<br>";
            echo "Image: <img src='" . $row['image'] . "' alt='Payment Image' style='max-width: 100px; max-height: 100px;'><br>";
            echo "Email: " . $row['email'] . "<br>";
            echo "Status: " . $row['status'] . "<br>";
        } else {
            echo "No payment information found for tracking number: $tid";
        }
    } else {
    ...
    header("Location: index.php"); // Change this to the actual page you want to redirect to
    exit();
}
...
?>
		...
		<script src="assets/js/bootstrapjsmin.js"></script>
		<!-- Map JS FILE -->
